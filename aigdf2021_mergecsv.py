...
data = []
for i in range(len(poi_filelist)):
    data.append(pd.read_csv(poi_filelist[i]))
data = pd.concat(data)

data = data.drop('Unnamed: 0', 1)
data.index = pd.Series(range(data.shape[0]))

myconv = LngLatTransfer()
for i in range(data.shape[0]):
    a,b = myconv.WGS84_to_BD09(data.loc[i, "wgslng"],data.loc[i, "wgslat"])
    c,d = myconv.WGS84_to_WebMercator(data.loc[i, "wgslng"],data.loc[i, "wgslat"])
    data.loc[i, "bdlat"],data.loc[i, "bdlng"] = a-0.000515, b+0.000472
    data.loc[i, "WMlat"],data.loc[i, "WMlng"] = c, d
